...
// node_modules/@mui/x-data-grid/internals/utils/warning.js
...
function GridFilterInputBoolean(props) {
  ...
  const [filterValueState, setFilterValueState] = React16.useState(item.value || "");
  const rootProps = useGridRootProps();
  const labelId = useId();
...
function useGridParamsApi(apiRef) {
  ...
  const getCellValue = React49.useCallback((id, field) => {
    const colDef = apiRef.current.getColumn(field);
    const row = apiRef.current.getRow(id);
    if (!row) {
      throw new MissingRowIdError(`No row with id #${id} found`);
...
var GridRootStyles = styled_default2("div", {
  ...
  const gridStyle = _extends({
    ...
    [`&.${gridClasses["root--noToolbar"]} [aria-rowindex="1"] .${gridClasses["columnHeader--last"]}`]: {
      borderTopRightRadius: !dimensions.hasScrollY || dimensions.scrollbarSize === 0 ? "calc(var(--unstable_DataGrid-radius) - 1px)" : void 0
    },
...
var useGridRowSelection = (apiRef, props) => {
  ...
  const removeOutdatedSelection = React133.useCallback(() => {
...
function GridColumnGroupHeader(props) {
  ...
  const rootProps = useGridRootProps();
  const headerCellRef = React138.useRef(null);
  const apiRef = useGridApiContext();
  ...
  const renderParams = React138.useMemo(() => ({
    groupId,
    headerName,
    description,
    depth,
  ...
  const publish = React138.useCallback(
    (eventName) => (event) => {
      if (isEventTargetInPortal(event)) {
        return;
      }
  ...
  const mouseEventsHandlers = React138.useMemo(() => ({
...
var useGridColumnHeaders = (props) => {
  ...
  const [dragCol, setDragCol] = React139.useState("");
  const [resizeCol, setResizeCol] = React139.useState("");
  const apiRef = useGridPrivateApiContext();
  const theme = useTheme();
  const rootProps = useGridRootProps();
  const dimensions = useGridSelector(apiRef, gridDimensionsSelector);
  ...
  const handleColumnResizeStart = React139.useCallback((params) => setResizeCol(params.field), []);
  const handleColumnResizeStop = React139.useCallback(() => setResizeCol(""), []);
  const handleColumnReorderStart = React139.useCallback((params) => setDragCol(params.field), []);
  const handleColumnReorderStop = React139.useCallback(() => setDragCol(""), []);
  const leftRenderContext = React139.useMemo(() => {
    return pinnedColumns.left.length ? {
      firstColumnIndex: 0,
      lastColumnIndex: pinnedColumns.left.length
    } : null;
  }, [pinnedColumns.left.length]);
  const rightRenderContext = React139.useMemo(() => {
  ...
  const getFillers = (params, children, leftOverflow, borderTop = false) => {
    ...
    return (0, import_jsx_runtime87.jsxs)(React139.Fragment, {
  ...
  const getColumnHeadersRow = () => {
    return (0, import_jsx_runtime87.jsxs)(GridColumnHeaderRow, {
      ...
      ownerState: rootProps,
      className: pinnedRows.top.length === 0 ? gridClasses["row--borderBottom"] : void 0,
...
var GridBaseColumnHeaders = React140.forwardRef(function GridColumnHeaders(props, ref) {
  const {
    className
...
var GridColumnHeaders2 = React141.forwardRef(function GridColumnHeaders3(props, ref) {
  const {
    visibleColumns,
    sortColumnLookup,
    filterColumnLookup,
...
var GridColumnUnsortedIcon = React143.memo(function GridColumnHeaderSortIcon2(props) {
  const {
    sortingOrder
...
function useProps(allProps) {
  return React145.useMemo(() => groupForwardedProps(allProps), [allProps]);
}
...
var useDataGridProps = (inProps) => {
  ...
  const localeText = React146.useMemo(() => _extends({}, GRID_DEFAULT_LOCALE_TEXT, themedProps.localeText), [themedProps.localeText]);
  const slots = React146.useMemo(() => computeSlots({
    defaultSlots,
    slots: themedProps.slots
  }), [themedProps.slots]);
  const injectDefaultProps = React146.useMemo(() => {
    return Object.keys(DATA_GRID_PROPS_DEFAULT_VALUES).reduce((acc, key) => {
      acc[key] = themedProps[key] ?? DATA_GRID_PROPS_DEFAULT_VALUES[key];
      return acc;
    }, {});
  }, [themedProps]);
  return React146.useMemo(() => _extends({}, themedProps, injectDefaultProps, {
    localeText,
    slots
...
var useGridRowsMeta = (apiRef, props) => {
  ...
  const rowsHeightLookup = React147.useRef(/* @__PURE__ */ Object.create(null));
  const lastMeasuredRowIndex = React147.useRef(-1);
  const hasRowWithAutoHeight = React147.useRef(false);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const filterModel = useGridSelector(apiRef, gridFilterModelSelector);
  ...
  const hydrateRowsMeta = React147.useCallback(() => {
    var _a, _b;
  ...
  const getRowHeight = React147.useCallback((rowId) => {
  ...
  const setRowHeight = React147.useCallback((id, height) => {
    rowsHeightLookup.current[id].sizes.baseCenter = height;
    rowsHeightLookup.current[id].isResized = true;
    rowsHeightLookup.current[id].needsFirstMeasurement = false;
    hydrateRowsMeta();
  }, [hydrateRowsMeta]);
  const debouncedHydrateRowsMeta = React147.useMemo(() => debounce(hydrateRowsMeta, props.rowPositionsDebounceMs), [hydrateRowsMeta, props.rowPositionsDebounceMs]);
  const storeMeasuredRowHeight = React147.useCallback((id, height) => {
  ...
  const rowHasAutoHeight = React147.useCallback((id) => {
    var _a;
    return ((_a = rowsHeightLookup.current[id]) == null ? void 0 : _a.autoHeight) || false;
  }, []);
  const getLastMeasuredRowIndex = React147.useCallback(() => {
    return lastMeasuredRowIndex.current;
  }, []);
  const setLastMeasuredRowIndex = React147.useCallback((index) => {
    if (hasRowWithAutoHeight.current && index > lastMeasuredRowIndex.current) {
      lastMeasuredRowIndex.current = index;
    }
  }, []);
  const resetRowHeights = React147.useCallback(() => {
...
var useGridStatePersistence = (apiRef) => {
  const exportState = React148.useCallback((params = {}) => {
    const stateToExport = apiRef.current.unstable_applyPipeProcessors("exportState", {}, params);
    return stateToExport;
  }, [apiRef]);
  const restoreState = React148.useCallback((stateToRestore) => {
...
var useGridColumnSpanning = (apiRef) => {
  ...
  const calculateColSpan = React149.useCallback(({
    rowId,
    minFirstColumn,
    maxLastColumn,
    columns
...
var useGridColumnGrouping = (apiRef, props) => {
  const getColumnGroupPath = React150.useCallback((field) => {
    const unwrappedGroupingModel = gridColumnGroupsUnwrappedModelSelector(apiRef);
    return unwrappedGroupingModel[field] ?? [];
  }, [apiRef]);
  const getAllGroupDetails = React150.useCallback(() => {
  ...
  const handleColumnIndexChange = React150.useCallback(() => {
    const unwrappedGroupingModel = unwrapGroupingColumnModel(props.columnGroupingModel ?? []);
    apiRef.current.setState((state) => {
      var _a;
  ...
  const updateColumnGroupingState = React150.useCallback((columnGroupingModel) => {
    var _a, _b;
...
var useGridColumnResize = (apiRef, props) => {
  ...
  const initialOffsetToSeparator = React151.useRef();
  const resizeDirection = React151.useRef();
  const stopResizeEventTimeout = useTimeout();
  const touchId = React151.useRef();
  ...
  const stopListening = React151.useCallback(() => {
    const doc = ownerDocument(apiRef.current.rootElementRef.current);
  ...
  const handleResizeStart = React151.useCallback(({
    field
  }) => {
    apiRef.current.setState((state) => _extends({}, state, {
  ...
  const handleResizeStop = React151.useCallback(() => {
    apiRef.current.setState((state) => _extends({}, state, {
      columnResize: _extends({}, state.columnResize, {
        resizingColumnField: ""
      })
  ...
  const isAutosizingRef = React151.useRef(false);
  const autosizeColumns = React151.useCallback(async (userOptions) => {
    var _a;
  ...
  React151.useEffect(() => stopListening, [stopListening]);
  useOnMount(() => {
...
var DataGridRaw = React152.forwardRef(function DataGrid(inProps, ref) {
...
var DataGrid2 = React152.memo(DataGridRaw);
